...
def can_form(obj, factors):
    ...
    if obj < 0:
        ...
        new_factors.pop(-1)
        n_digits = len(str(this_factor))
        res = (
            can_form(obj - this_factor, new_factors)
            or (
                (obj % this_factor == 0)
                and can_form(round(obj / this_factor), new_factors)
            )
            or (
                (obj % (10**n_digits) == this_factor)
                and can_form(obj // (10**n_digits), new_factors)
            )
        )
